---
title: "MMM model using Robyn - Capstone Group Project"
author: AdFlow
date: "2024-01-28"
...
# Tips: any events can be added into this table, school break, events etc.
...
#robyn_directory <- "C:/Users/mutaps2/OneDrive - Medtronic PLC/Desktop/Adflow/"
robyn_directory <- "/Users/oliviapoon/Documents/Stern Note/Capstone/repo/adflow/MMM/"

df <- data.table::fread(file.path(robyn_directory, "weekly_aggregated_data.csv")) 

# df <- read.csv(file.path(robyn_directory, "MMM_Input.csv")) %>%
#         mutate(Date = as.Date(Date, "%Y-%m-%d"),
#                week = strftime(Date, format = "%U %Y")) %>%
#         group_by(week) %>%
...
```{r}
## Data Dictionary
readxl::read_xlsx(file.path(robyn_directory, "MMM Data Dictionary.xlsx")) %>% DT::datatable()
```

```{r}
ggplot(df,aes(x = Date)) +
  geom_line(aes(y = M_A_FTV_DIS_SPEND, color = "Awareness Fire TV Display")) +
  geom_line(aes(y=M_A_SA_SPEND, color = "Awareness Streaming TV")) +
  geom_line(aes(y=M_C_OLV_SPEND, color = "Consideration Online Video")) +
  geom_line(aes(y=M_C_DSP_DIS_SPEND, color = "Consideration DSP Display")) +
  geom_line(aes(y=M_P_DSP_DIS_SPEND, color = "Purchase DSP Display")) +
  geom_line(aes(y=M_P_SP_SPEND, color = "Purchase Sponsored Products")) +
  geom_line(aes(y=M_P_OO_SA_SPEND, color = "Purchase O&O Display")) +
  ylab("SPEND ($)")
```
...
InputCollect <- robyn_inputs(
  ...
  dep_var_type = "revenue", #"conversion", # "revenue" (ROI) or "conversion" (CPA)
  prophet_vars = c("trend", "season", "holiday"), # "trend","season", "weekday" & "holiday"
  prophet_country = "US", # input country code. Check: dt_prophet_holidays
  context_vars = c("Competitor_Sales1", "Competitor_Sales2"), # e.g. competitors, discount, unemployment etc
  # context_signs = c("positive", "positive"), 
  paid_media_spends = c("M_A_FTV_DIS_SPEND", "M_A_SA_SPEND", "M_C_OLV_SPEND", 
                        "M_C_DSP_DIS_SPEND", "M_P_DSP_DIS_SPEND", "M_P_SP_SPEND", 
                        "M_P_OO_SA_SPEND"), # mandatory input
  paid_media_vars = c("M_A_FTV_DIS_IMP", "M_A_SA_IMP", "M_C_OLV_IMP", 
                      "M_C_DSP_DIS_IMP", "M_P_DSP_DIS_IMP", "M_P_SP_IMP", 
                      "M_P_OO_SA_IMP"), # mandatory.
  paid_media_signs = c("positive", "positive", "positive", 
                      "positive", "positive", "positive", 
                      "positive"), # signs of coefficients for paid_media_vars
...
**!! may want to review below !! **
OP 2024-01-30: not very sure how to set the bounds...
...
hyperparameters <- list(
  # Awareness Fire TV Display
  M_A_FTV_DIS_SPEND_alphas = c(0.5, 3),
  M_A_FTV_DIS_SPEND_gammas = c(0.3, 1),
  M_A_FTV_DIS_SPEND_thetas = c(0, 0.3),
  
  # Awareness Streaming TV
  M_A_SA_SPEND_alphas = c(0.5, 3),
  M_A_SA_SPEND_gammas = c(0.3, 1),
  M_A_SA_SPEND_thetas = c(0.1, 0.4),
  
  # Consideration DSP Display
  M_C_DSP_DIS_SPEND_alphas = c(0.5, 3),
  M_C_DSP_DIS_SPEND_gammas = c(0.3, 1),
  M_C_DSP_DIS_SPEND_thetas = c(0.3, 0.8),
  
  # Consideration Online Video
  M_C_OLV_SPEND_alphas = c(0.5, 3),
  M_C_OLV_SPEND_gammas = c(0.3, 1),
  M_C_OLV_SPEND_thetas = c(0, 0.3),
  
  # Purchase DSP Display
  M_P_DSP_DIS_SPEND_alphas = c(0.5, 3),
  M_P_DSP_DIS_SPEND_gammas = c(0.3, 1),
  M_P_DSP_DIS_SPEND_thetas = c(0.1, 0.4),
  
  # Purchase O&O Display
  M_P_OO_SA_SPEND_alphas = c(0.5, 3),
  M_P_OO_SA_SPEND_gammas = c(0.3, 1),
  M_P_OO_SA_SPEND_thetas = c(0.1, 0.4),
  
  # Purchase Sponsored Products
  M_P_SP_SPEND_alphas = c(0.5, 3),
  M_P_SP_SPEND_gammas = c(0.3, 1),
  M_P_SP_SPEND_thetas = c(0.1, 0.4),
...
select_model <- "5_68_2" #"2_229_4" #OutputCollect$allSolutions[1]
```
...
### Example 3 (OP attempt, budget = 1M)
```{r}
# Scenario "target_efficiency": "How much to spend to hit ROAS or CPA of x?"
# Example 3: Use default ROAS target for revenue or CPA target for conversion
# Check InputCollect$dep_var_type for revenue or conversion type
